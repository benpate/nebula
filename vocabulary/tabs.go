...
func (w Tabs) Init(c *content.Content, id int) {

	me := c.GetItem(id)

	// Let's add THREE new tabs
	for index := 1; index <= 3; index++ {
		itemID := c.NewItem(w.library, ItemTypeContainer) // Make a new container
		item := c.GetItem(itemID)                         // Get a pointer to the item in the content structure
...
func (w Tabs) View(b *html.Builder, c content.Content, id int) {
...
func (w Tabs) Edit(b *html.Builder, c content.Content, id int, endpoint string) {
	...
	for index, id := range item.Refs {
		...
		w.library.Edit(b, c, id, endpoint)
		b.Close()
	}
